...
<?php
  ...
  if(mysqli_connect_errno()) {
    ...
    if(isset($_POST['delete'])) {
      $game = $_POST['game'];
      $deleteResult = "delete from results where game='$game'";
      mysqli_query($con, $deleteResult);
      echo "<script>alert('Result succesfully deleted!');</script>";
    }
          ...
          <div class="row">
            <div class="animated fadeIn">
              <div class="jumbotron jumbotron-fluid">
                <div class="container">
                  <h1 class="display-3">Look at them results.</h1>
                  <p class="lead">Here you can view the results so far this Sportsfest. You can also edit or delete results.</p>
                </div>
              </div>
              <div class="col-lg-18">
                <table class="table table-responsive-sm">
                  ...
                  <?php
                    $getResultData = "select * from results";
                    $resultData = mysqli_query($con, $getResultData);
                    while($data = mysqli_fetch_array($resultData)) {
                      ...
                      echo "<td>";
                    ?>
                    <form method='POST' action='admin-addresult.php' style="display: inline">
                      <input type="hidden" value="<?php echo $data['game']; ?>" name="name"/>
                      <input type="hidden" value="<?php echo $data['type']; ?>" name="type"/>
                      <?php
                        for ($i = 1; $i <= $numTeams; $i++) {   
                          if (($i %100) >= 11 && ($i%100) <= 13) {
                            $number = $i. 'th';
                            echo "<input type='hidden' value='$data[$number]' name='$number'/>";
                          }
                          else {
                            $number = $i. $ends[$i % 10];
                            echo "<input type='hidden' value='$data[$number]' name='$number'/>";
                    ...
                    <form method='POST' action='<?php $_PHP_SELF; ?>' style="display: inline" >
                      <input type="hidden" value="<?php echo $data['game']; ?>" name="game"/>
